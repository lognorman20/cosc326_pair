...
class Ant:
    ...
    DIR_CHANGES = {"N": (0, 1), "E": (1, 0), "S": (0, -1), "W": (-1, 0)}

    @staticmethod
    def get_idx(dir):
        """
        Gets the index of a cardinal direction in the array ['N','E','S','W']
        Returns an integer for the index if successful, else -1
        """
    ...
    def getPositionChange(moves):
        """
        Given a list of moves (N, E, S, W characters), this function returns the
        change in position that would result from following the moves in order.
        """
        x, y = 0, 0
    ...
    def fastFoward(self, loopLen):
        """
        Given the number of steps back from the current position that the loop
        starts, this function calculates and sets the final position of the ant
        if it were to repeat the loop until running out of moves.
        """
        numExtraMoves = self.num_moves % loopLen
        numFullLoops = int((self.num_moves - numExtraMoves) / loopLen)
        fullLoopChange = (
            self.prevStates[-1].position[0] - self.prevStates[-1 - loopLen].position[0],
            self.prevStates[-1].position[1] - self.prevStates[-1 - loopLen].position[1],
        )
        extraMovesChange = (
            self.prevStates[-1 - loopLen + numExtraMoves].position[0]
            - self.prevStates[-1 - loopLen].position[0],
            self.prevStates[-1 - loopLen + numExtraMoves].position[1]
            - self.prevStates[-1 - loopLen].position[1],
        )
        self.position = (
            self.position[0] + fullLoopChange[0] * numFullLoops + extraMovesChange[0],
    ...
    def detectStationaryLoop(self, loopLen):
        """
        Given a repeating set of moves of length [loopLen], this function checks
        if the ant is in a stationary loop.  If the ant repeats the same set of
        moves and ends up where it started, it must go on forever!
        """
        return (
            self.prevStates[-loopLen].position == self.prevStates[-2 * loopLen].position
        )

    def detectHighwayLoop(self, loopLen):
        """
        Given a repeating set of moves of length [loopLen], this function checks
        if the ant is in a highway loop.  If the ant repeats the same moves,
        setting the same colors, and it does this until the entire loop is
        further away from the origin than anything before it, its path is
        unobstructed and it will go on forever!
        """
        psLength = len(self.prevStates)
        latestLoopMinRadius = min(
            [
                prevState.position[0] ** 2 + prevState.position[1] ** 2
                for prevState in islice(self.prevStates, psLength - loopLen, psLength)
            ]
        )
        currentLoopIndex = psLength - loopLen - 1
        while currentLoopIndex > loopLen * 2 and self.detectLoop(
            psLength - currentLoopIndex, loopLen
        ):
    ...
    def detectLoop(self, stepsBack, loopLen):
        """
        Do the [loopLen] moves ending [stepsBack] moves ago match the [loopLen]
        moves that came before them?
        """
        for i in range(0, loopLen):
            if (
                self.prevStates[-1 - stepsBack - i].direction
                != self.prevStates[-1 - stepsBack - i - loopLen].direction
                or self.prevStates[-1 - stepsBack - i].colorSet
                != self.prevStates[-1 - stepsBack - i - loopLen].colorSet
            ):
                return False
        return True
    ...
    def findLoops(self):
        ...
        for i in range(1, int(len(self.prevStates) / 2)):
            if self.detectLoop(0, i) and (
                self.detectStationaryLoop(i) or self.detectHighwayLoop(i)
            ):
                return i
        return None
    ...
    def move(self):
        ...
        Returns a tuple of the final position after moving
        """
        while self.num_moves > 0:
            if not self.simpleMode and self.num_moves == self.doLoopDetectionAtNumMoves:
                ...
                self.doLoopDetectionAtNumMoves = self.num_moves - int(
                    self.loopDetectionInterval
                )
            ...
            if not self.simpleMode:
                self.maxRadius = max(
                    self.maxRadius, self.position[0] ** 2 + self.position[1] ** 2
                )
                self.prevStates.append(
                    AntState(self.curr_dir, set_state, self.maxRadius, self.position)
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simulates the movement of Langton's Ant"
    )
    parser.add_argument(
        "filename", type=str, help="The file containing ant instructions"
    )
    parser.add_argument(
        "-i",
        "--image_dir",
        type=str,
        help="The directory to save the output images to. Specifying this "
        "option will disable loop detection and fast forwarding.",
    )
    parser.add_argument(
        "-s",
        "--simple",
        action="store_true",
        help="Disable loop detection and fast forwarding",
    )
